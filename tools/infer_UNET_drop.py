...
def infer(config_path):
    ...
    # Load LDM model
    model_dir = Path("./result/LDM_CELEB_IMAGE_TEXT/")
    model_file = model_dir / 'LDM.pt'
    ...
    if 'class' in config['infer_condition']:
    ...
    if 'image' in config['infer_condition'] or 'text' in config['infer_condition']:
    ...
    if 'image' in config['infer_condition']:
    ...
    # If image is not in the infer condition, the model still expect the mask
    # Therefore, creative negative instance to feed into the model
    else:
        temp_mask_image = mask_dataset[0][1]['image'][None, ...]
        mask_image = torch.zeros_like(temp_mask_image).repeat(config['num_samples'],1,1,1)
        mask_image = mask_image.to(device)
        negative_mask_image = mask_image
    ...
    if 'text' in config['infer_condition']:
    ...
    # If text is not in the infer condition, the model still expect the text
    # Therefore, creative negative instance to feed into the model
    else:
